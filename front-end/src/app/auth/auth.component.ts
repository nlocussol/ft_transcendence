...
export class AuthComponent implements OnInit {
  ...
  async sendUserData(data: any) {
    ...
    this.http
      .post('http://localhost:3000/db-writer/create-user/', body, { headers })
      .subscribe();
  ...
  async getUserData(accessToken: string) {
    ...
    const res: any = await this.http
      .get('https://api.intra.42.fr/v2/me', { headers })
      .toPromise();
    console.log(res);
    ...
    this.http
      .post('http://localhost:3000/auth/login', { login: this.login })
      .subscribe(() => {
  ...
  async getAccessToken(code: string) {
    const body = new URLSearchParams({
      ...
      client_id:
        'u-s4t2ud-d4f9852c6392f3a567c8fb78fac0ffaa6a248187093e5a84ba0a0b1e507c8f01',
      client_secret:
        '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',
      code: code,
    ...
    const res: any = await this.http
      .post('https://api.intra.42.fr/oauth/token', body.toString(), { headers })
      .toPromise();
    this.getUserData(res.access_token);
  }

  login2(code: string) {
    this.authService.retrieveAccessToken(code as string).subscribe({
