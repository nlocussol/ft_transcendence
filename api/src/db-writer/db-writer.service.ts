...
export class DbWriterService {
  ...
  async fillMatchHistory(gameData: GameData) {
    ...
    if (gameData.players[0].score == 5) matchWinner = gameData.players[0].login;
    else matchWinner = gameData.players[1].login;
  ...
  async getLeaderboard() {
    ...
    const sortedPlayer = players.sort((a, b) =>
      a.stats.win - a.stats.lose > b.stats.win - b.stats.lose ? -1 : 1,
    );
    return sortedPlayer;
  }
